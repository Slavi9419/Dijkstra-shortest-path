...
public class FindAndVisualizeMostShortestPath {
	...
	private List<String> towns = new ArrayList<>(graph.getMyGrapht().vertexSet());
	...
	private ImageIcon map = new ImageIcon(this.getClass().getClassLoader().getResource("Data/Map/bgMap.png"));
	...
	private void FaindShorthPath() {
		...
		if (!graph.getMyGrapht().containsVertex(startVertex) || !graph.getMyGrapht().containsVertex(endVertex)) {
		...
		for (int i = 0; i < towns.size(); i++) {
			if (i == 0) {
				for (int j = 0; j < length; j++) {
					if (!visited[j] && graph.getMyGrapht().containsEdge(towns.get(i), towns.get(j))
							&& shortDist[j] > shortDist[i] + graph.getMyGrapht().getEdge(towns.get(i), towns.get(j))) {

						shortDist[j] = shortDist[i] + graph.getMyGrapht().getEdge(towns.get(i), towns.get(j));
						path[j] = i;
						pathLines[j] = (graph.getMyGrapht().getEdge(towns.get(i), towns.get(j)));
				...
				for (int l = 0; l < length; l++) {
					if (!visited[l] && graph.getMyGrapht().containsEdge(towns.get(min), towns.get(l))) {
						if (l == length - 1) {
							visited[min] = true;
						}
						if (shortDist[l] > shortDist[min]
								+ (graph.getMyGrapht().getEdge(towns.get(min), towns.get(l)))) {

							shortDist[l] = shortDist[min] + (graph.getMyGrapht().getEdge(towns.get(min), towns.get(l)));
							path[l] = min;
							pathLines[l] = (graph.getMyGrapht().getEdge(towns.get(min), towns.get(l)));
	...
	public void show() {
		...
		mapFrame.setSize(600, 900);
		mapFrame.setExtendedState(JFrame.MAXIMIZED_BOTH);
		mapFrame.getContentPane().add(graphAdapter);
		ImageIcon img = new ImageIcon(this.getClass().getClassLoader().getResource("Data/Logo/logo.png"));
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (newCountry.equals("Germany")) {
						graph.graphFactory.setCountryVertexFile(this.getClass().getResourceAsStream("/Data/vertexGB.txt"));
						graph.graphFactory.setCountryEdgesFile(this.getClass().getResourceAsStream("/Data/edgesGB.txt"));
						map = new ImageIcon(this.getClass().getClassLoader().getResource("Data/Map/gbMap.png"));

					} else if (newCountry.equals("Bulgaria")) {
						graph.graphFactory.setCountryVertexFile(this.getClass().getResourceAsStream("/Data/vertexBG.txt"));
						graph.graphFactory.setCountryEdgesFile(this.getClass().getResourceAsStream("/Data/edgesBG.txt"));
						map = new ImageIcon(this.getClass().getClassLoader().getResource("Data/Map/bgMap.png"));

					} else if (newCountry.equals("Macedonia")) {
						graph.graphFactory.setCountryVertexFile(this.getClass().getResourceAsStream("/Data/vertexMK.txt"));
						graph.graphFactory.setCountryEdgesFile(this.getClass().getResourceAsStream("/Data/edgesMK.txt"));
						map = new ImageIcon(this.getClass().getClassLoader().getResource("Data/Map/mkMap.png"));

					} else if (newCountry.equals("Russia")) {
						graph.graphFactory.setCountryVertexFile(this.getClass().getResourceAsStream("/Data/vertexRU.txt"));
						graph.graphFactory.setCountryEdgesFile(this.getClass().getResourceAsStream("/Data/edgesRU.txt"));
						map = new ImageIcon(this.getClass().getClassLoader().getResource("Data/Map/ruMap.png"));

					} else if (newCountry.equals("USA")) {
						graph.graphFactory.setCountryVertexFile(this.getClass().getResourceAsStream("/Data/vertexUS.txt"));
						graph.graphFactory.setCountryEdgesFile(this.getClass().getResourceAsStream("/Data/edgesUS.txt"));
						map = new ImageIcon(this.getClass().getClassLoader().getResource("Data/Map/usMap.png"));

					} else if (newCountry.equals("Canada")) {
						graph.graphFactory.setCountryVertexFile(this.getClass().getResourceAsStream("/Data/vertexCA.txt"));
						graph.graphFactory.setCountryEdgesFile(this.getClass().getResourceAsStream("/Data/edgesCA.txt"));
						map = new ImageIcon(this.getClass().getClassLoader().getResource("Data/Map/caMap.png"));
					...
					towns = new ArrayList<>(graph.getMyGrapht().vertexSet());
					inputVertex = false;
					graphAdapter = buildUrbanNetwork();
					length = towns.size();
	...
	private JGraph buildUrbanNetwork() {
		final JGraph b = new JGraph(new JGraphModelAdapter<String, Double>(graph.getMyGrapht()));
		b.setBackgroundImage(map);
		GraphLayoutCache cache = b.getGraphLayoutCache();
		int width = 90;
		int height = 30;

		for (Object item : b.getRoots()) {
		...
		b.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				System.out.println(b.getMousePosition());
				try {
					if (!graph.getMyGrapht().containsVertex(b.getSelectionCell().toString())) {
						return;
					}
				...
				if (!inputVertex) {
					inputStartVertex.setText(b.getSelectionCell().toString());
					if (!inputEndVertex.getText().equals("")) {
						inputEndVertex.setText("");
					}
					inputVertex = true;
				} else {
					inputEndVertex.setText(b.getSelectionCell().toString());
					inputVertex = false;
	...
	private JTextArea addTextArea() {
		...
		for (String v : graph.getMyGrapht().vertexSet()) {
			textArea.append(v);
			textArea.append("\n");
		}
